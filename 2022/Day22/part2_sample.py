...
for file_name in ["sample_p2.txt"]:
    ...
    if file_name == "input_p2.txt":
        right.rotate()
        left.rotate()
        back.rotate()
        back.rotate()
        back.rotate()
    elif file_name == "sample_p2.txt":
        back.mirror()
        back.rotate()
        back.rotate()
        right.rotate()
    ...
    for i in range(size):
        ...
        face.grid[i][0].left = (left.grid[size - 1 - i][-1], LEFT)
        left.grid[size - 1 - i][-1].right = (face.grid[i][0], RIGHT)

        face.grid[-1][i].bottom = (bottom.grid[0][i], BOTTOM)
        bottom.grid[0][i].top = (face.grid[-1][i], TOP)

        face.grid[i][-1].right = (right.grid[i][0], RIGHT)
        right.grid[i][0].left = (face.grid[i][-1], LEFT)

        # ============== BACK ================
        back.grid[-1][i].bottom = (top.grid[0][i], BOTTOM)
        top.grid[0][i].top = (back.grid[-1][i], TOP)

        back.grid[0][i].top = (bottom.grid[-1][i], TOP)
        bottom.grid[-1][i].bottom = (back.grid[0][i], BOTTOM)

        back.grid[i][0].left = (left.grid[size - 1 - i][0], RIGHT)
        left.grid[size - 1 - i][0].left = (back.grid[i][0], RIGHT)

        back.grid[i][-1].right = (right.grid[size - 1 - i][-1], LEFT)
        right.grid[size - 1 - i][-1].right = (back.grid[i][-1], LEFT)

        # ============== TOP ================
        top.grid[i][0].left = (left.grid[0][i], BOTTOM)
        left.grid[0][i].top = (top.grid[i][0], RIGHT)

        top.grid[i][-1].right = (right.grid[0][size - 1 - i], BOTTOM)
        right.grid[0][size - 1 - i].top = (top.grid[i][-1], LEFT)

        # ============== BOTTOM ================
        bottom.grid[i][0].left = (left.grid[-1][size - 1 - i], TOP)
        left.grid[-1][size - 1 - i].bottom = (bottom.grid[i][0], RIGHT)

        bottom.grid[i][-1].right = (right.grid[-1][i], TOP)
        right.grid[-1][i].bottom = (bottom.grid[i][-1], LEFT)
